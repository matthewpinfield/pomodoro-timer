...
export function PieChart({
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const maxWidth = Math.min(width * 0.8, 380); // Reduced from 420 to 380
      
      if (width < 480) { // Mobile breakpoint
        setChartSize({ 
          width: Math.min(260, maxWidth), // Reduced from 280 to 260
          height: Math.min(260, maxWidth) // Reduced from 280 to 260
        });
      } else if (width < 640) { // Small breakpoint
        setChartSize({ 
          width: Math.min(280, maxWidth), // Reduced from 300 to 280
          height: Math.min(280, maxWidth) // Reduced from 300 to 280
        });
      } else { // Medium and up
        setChartSize({ 
          width: Math.min(380, maxWidth), // Reduced from 420 to 380
          height: Math.min(380, maxWidth) // Reduced from 420 to 380
        });
      }
    }
  ...
  useEffect(() => {
    try {
      ...
      const totalGoalTime = displayTasks.reduce((sum, task) => sum + task.goalTimeMinutes, 0);
      ...
      displayTasks.forEach((task, index) => {
        ...
        const isHovered = hoveredSlice === index && tasks.length > 0; // Only allow hover effects with real tasks
      ...
      // Different center text for demo vs. real tasks
      if (tasks.length === 0) {
        ctx.fillText("Task Progress", centerX, centerY - 10);
      } else {
        ctx.fillText("Today", centerX, centerY - 10);
      }

      // If a task is selected or hovered, show its name
      if ((hoveredSlice !== null || selectedSlice !== null) && tasks.length > 0) {
        ...
        // Different label for demo vs. real tasks
        if (tasks.length === 0) {
          ctx.fillText("Click to add new items", centerX, centerY + 15);
        } else {
  ...
  }, [tasks, hoveredSlice, selectedSlice, chartSize, isHoveringCenter, workdayHours, workdayMinutes, demoTasks]);
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-full p-4">
      ...
      {/* Task count and workday indicator */}
      {tasks.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex items-center text-sm text-gray-600 mb-2 mt-2"
        >
